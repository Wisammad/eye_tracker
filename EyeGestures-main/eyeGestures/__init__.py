...
class EyeGestures_v3:
    ...
    def step(self, frame, calibration, width, height, context="main"):
        ...
        # Prepare for moving to next calibration point if needed
        ...
        current_point = self.clb[context].getCurrentPoint(width,height)
        calibration_points_count = len(self.clb[context].clbMtx.points)
        point_idx = self.clb[context].clbMtx.iterator
        
        cevent = Cevent(
            point=current_point,
            acceptance_radius=self.clb[context].acceptance_radius, 
            calibration_radius=self.clb[context].calibration_radius,
            calibration_target=current_point,
            calibration_point_idx=point_idx,
            calibration_points=calibration_points_count,
